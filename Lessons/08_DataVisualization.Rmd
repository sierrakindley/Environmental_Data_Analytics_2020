...
PP.temp <-
    ggplot(subset(PeterPaul.chem.nutrients, depth == 0)) + #need to subset dataset by calling for depth = 0 in order to get only surface temperatures
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #adds line of best fit to plot; could add ', se = FALSE' in parentheses after 'lm'
  geom_abline(aes(slope = 16, intercept = 0)) #adds AB line that we define (we specify slope and intercept) to plot
print(NvsP)
...
Nutrientplot6 <-
  ...
  geom_freqpoly(aes(x = po4), color = "red") #have to make a separate layer for each column you want to plot
print(Nutrientplot6)
