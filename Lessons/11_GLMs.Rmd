...
Litter.Totals <- Litter %>%
  ...
  summarise(dryMass = sum(dryMass)) #add up totals of litter mass for each of the individual groupings we have specified

# Test for normality (normal distribution of data)
...
# Test for equal variance -- low p-value = values not at all close to one another
# ANOVA is robust against departures from equal variance.
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID)

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) #predicting dryMass as y and plotID as alpha variables in y = (Beta)x + Alpha + Error equation
summary(Litter.Totals.anova)

# Format ANOVA as lm -- only compares each variable to the intercept (not to one another)??; the 'intercept' is whichever variable is listed first in the dataset (e.g. the first plotID listed, etc.)
...
# Post-hoc test
TukeyHSD(Litter.Totals.anova) #can only run on your 'aov' object (line 66); if any letters match, there are not significantly different values between them
...
> Posed question: Does total litter biomass differ among sites at the Niwot Ridge LTER? 
We found: Total litter biomass differed significantly among sites at the Niwot Ridge LTER (figure 1; ANOVA, F11,198 = 4.183, p < 0.0001).
* The further away the F-statistic is from zero, the more likely you are to find significantly different groupings in your data.
...
```{r}
# Format as aov
Litter.anova.2way <- aov(data = Litter, dryMass ~ functionalGroup + nlcdClass) # '+' means we are testing the effect of functionalGroup and nlcdClass separately
summary(Litter.anova.2way) # shows us that both functionalGroup and nlcdClass are significant predictors of biomass
...
```{r}
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass) # '*' denotes we are testing interaction effects of functionalGroup and nlcdClass on dataset; 'functionalGroup:nlcdClass' p-value less than 0.05 denotes a significant interaction effect between the two predictor variables
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  xlab(expression(paste("NLCD Class"))) + 
  ylab(expression(paste("Dry Mass (g)"))) +
  labs(color = "Functional Group") +
    stat_summary(geom = "text", fun.y = max, position = position_dodge(width=preserve = "total"), vjust = -1, size = 3.5,
               label = c("d", "d", "d", "a", "d", "d", "d", "d", "d", "d", "d", "b", "d", "d", "d", "d", "d", "d", "cd", "bc", "d", "d", "d",
                         "d"))
