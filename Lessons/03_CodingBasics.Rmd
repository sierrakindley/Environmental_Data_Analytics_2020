...
vector1[3] #calls up third object in vector1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #numbers increase as you go down a column and then to the next column to the right
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #numbers increase as you go right along a row and then to the next row down
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #assigns names to rows and columns, respectively

matrix1[4, ] #calls up the fourth row of matrix1
matrix1[ , 3] #calls up the third column of matrix1
matrix1[c(12, 14)] #calls up the 12th and 14th objects in matrix1
matrix1[c(12:14)] #calls up the objects between the 12th and 14th objects in matrix1, including the 12th and 14th objects themselves
matrix1[2:4, 1:3] #calls up rows 2 through 4 and columns 1 through 3 of matrix1 (also called matrix subsetting)
...
dataframe1[1:2,] # 
dataframe1[c("ID","Passed")] #calls up the elements (columns) called "ID" and "Passed" in dataframe1
dataframe1$ID

```
Question: How do the different types of data appear in the Environment tab?

> Answer: There are data and values categories in the Environment tab. The data are subdivided by dataframe, list, and matrix.

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: It autofills the function/command with the first element (column) listed (which is "ID")
...
```{r}
hightempF <- c(62,45,50,50,40,40,45,52,54,58)
lowtempF <- c(31,29,43,24,23,23,26,30,42,41)
...
```{r}
hightempC <- (hightempF-32)*(5/9)
lowtempC <- (lowtempF-32)*(5/9)
...
```{r}
dataframe2 <- data.frame(hightempF,lowtempF,hightempC,lowtempC)
names(dataframe2) <- c("High Temp F", "Low Temp F", "High Temp C", "Low Temp C")
...
```{r}
summary(dataframe2) #calculates summary statistics for entire dataframe2
sd(dataframe2[,1]) #calculates SD for 1st column in dataframe2
...
> Answer: It shows you which packages have been installed and if there are any conflicting commands/functions that exist in other packages (commands/functions that are called the same thing)
