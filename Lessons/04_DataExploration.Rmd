...
# Relative file path (friendly for users regardless of machine)...staring name in parentheses with one dot tells R to go in one folder level, while beginning with two dots tells R to go out one folder level--then the new folder(s) you want to enter is specified
...
> ANSWER: This file can be found in the metadata folder. Metadata provides more useful infomration about the dataset than the data file itself. This information is useful in the interpretation, understanding, and use of the data. Also, having access to the web source of the data via the metadata file is useful to be able to go back to the original source of the data. This metadata file also allows you to see the status of the data (approved, provisional, etc.) at the time of download. In addition, the information in this file is labeled with units and uses a defined naming convention (which is explained in the metadata file).
...
colnames(USGS.flow.data) <- c("agency_cd", "site_no", "datetime", 
                              ...
                              "gage.height.mean", "gage.height.mean.approval") #try not to exceed 80 characters per line of code
str(USGS.flow.data)
dim(USGS.flow.data)
...
> Answer: Blank cells that are part of numeric vectors/variables are now listed as NAs in R. Blank cells part of factor vectors/variables remain blank in R.
...
> Answer: Retain NAs: future reference (I need more site data here, when...); Remove NAs: calculations, data analysis based on presence data only (vs. presence and absence data)

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #If you are making permanent changes to your dataset (which we are doing here), it is wise to call the resulting dataset something else/new (here, it is 'USGS.flow.data.complete')
dim(USGS.flow.data)
...
In some cases when dates are provided as integers, you may need to provide an origin for your dates. Beware: the "origin" date for Excel (Windows), Excel (Mac), R, and MATLAB all have different origin dates. Google this if it comes up. In R, this origin date is January 1, 1970.
