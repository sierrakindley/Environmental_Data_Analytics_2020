...
>  This test yields a distribution that is not a normal distribution. It is skewed to the left. Thus, generally, in the context of the research question, Ozone levels are collectively below the threshold for "good" AQI index (0-50).
...
shapiro.test(EPAair$Ozone[EPAair$Year == 2019]) #null hypothesis (for lines 93 & 94): sample is well approximated by a normal distribution (evidenced by very low p-values)
...
# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) #testing ozone BY year (~ = by) 
O3.twosample #two-sample t-test; rejected null hypothesis that these means are equal
...
densityplot1 <- ggplot(EPAair, aes(x = Ozone/Year, color = as.factor(Year))) +
  geom_density()
print(densityplot1)

densityplot2 <- ggplot(EPAair, aes(x = Year, y = Ozone/Year, color = as.factor(Year))) +
  geom_boxplot()
print(densityplot2)

densityplot3 <- ggplot(EPAair, aes(x = Year, y = Ozone/Year, color = as.factor(Year))) +
