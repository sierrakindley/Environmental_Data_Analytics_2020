...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) #generates daily data for the 8th column of the dataset
```
...
```{r}
?stl #brings up help file for stl function
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed) #trend = "moving average" of sorts; remainder = random component of data; season = period containing number of days (unit of time) that you specified

# We can extract the components (from the plot generated above) and turn them into data frames
...
```{r}
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) #shapiro test can only process 5000 samples, so need to subset early data
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000) #shapiro test can only process 5000 samples, so need to subset late data
...
wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) #shows that discharge data from our two different time periods (early and late) are significantly different from one another--can confirm the alternative hypothesis (because you're rejecting the null, as the p-value is less than 0.05 for the alternative hypothesis)

summary(EnoDischarge.early$discharge.mean)
summary(EnoDischarge.late$discharge.mean)

#time series = continuous response over time (of a response variable)
```

How might you interpret the results of this test, and how might you represent them graphically?

> Mean daily discharge in the Eno River from 1927-1971 was significantly higher than mean daily discharge from 1985-2019 (Wilcoxon test, W = 1.17*10^8, p < 0.0001). Use a box plot to graphically represent these results.
...
> aggregation: Bundling data (from mulitple datasets) and then scaling it up; Might be useful when you have a lot of data

> subsampling: Taking a smaller sample of data from an existing dataset; Might be useful if there is a lot of data (could take a subsample that is representative of the larger dataset) 

> interpolation: Using existing data to predict/"fill in the gaps" where there are data missing
...
summary(EnoDischarge.late.trend) #no significant trend across months/"seasons" (p-value = 0.8889)
...
> There is no significant trend in mean discharge in the Eno River across months (SMK, p-value > 0.05, z = -0.13967). Mean discharge in the Eno River does not have a significant seasonal trend (SMK, p-value > 0.05, z = -0.13967).
