...
# add your code here
class(basins_nf_seplains_raw) #class is 'sf' and 'data.frame'
colnames(basins_nf_seplains_raw) #column names are 'AREA', 'PERIMETER', 'GAGE_ID', and 'geometry'
length(names(basins_nf_seplains_raw)) #there are 4 columns
...
st_crs(basins_nf_seplains_raw) #verify to see if Coordinate Reference System and EPSG were changed to what was specified
...
NC_state_bounds_geom <-
  southeast_state_bounds %>%
    ...
    st_geometry() #drops the attribute data
...
NC_centroid <- st_centroid(NC_state_bounds_geom) #find centroid of NC
...
ggplot() +
  geom_sf(data = southeast_state_bounds) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines; can add comma after data declaration and put 'aes()' to add aesthetics
...
ggplot() +
  geom_sf(data = NC_state_bounds_geom) + #this will be the base or bottom layer of the map; the first dataset listed will be the base or bottom   layer
...
nc_state_bounds_geom <- southeast_state_bounds %>%
  filter(NAME == "North Carolina") %>%
  st_geometry() #select just the geometry and drop the attribute data; helps mapping process go more quickly in R (they will load more    quickly)
...
nc_bbox <- nc_state_bounds_geom %>%
  ...
  st_transform(4326) %>% # WGS84 (for lat and long); change data back to lat., long. because the basemap we want to use only likes lat., long.   format
  st_bbox()
nc_bbox # check
