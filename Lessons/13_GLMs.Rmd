...
summary(TNancova.main) #intercept represents total nitrogen concentration in Paul Lake at 0m depth (intercept represents group listed first--Paul Lake in this case)

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction) #intercept represents surface level in Paul Lake (again)
...
NTL.summertemp <- 
  ...
  #code won't work if there are NAs--excludes all rows with NAs
  na.exclude()
...
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, #run temperature by year
                     random = ~1|lakename)
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  ...
  theme(legend.spacing.x = unit(0, "cm")) #makes lake labels and color dots in legend a little closer together
print(NTL.tempmodel)
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: There is statistically significant change (increase) in surface temperature in northern Wisconsin lakes over time.
