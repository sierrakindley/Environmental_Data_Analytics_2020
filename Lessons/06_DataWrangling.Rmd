...
> Add notes here: Could reveal climate change impacts, could help explain changes in organism populations, could shed light on ecosystem health, results could impact policy
...
str(NTL.phys.data) #asks for structure of dataset
dim(NTL.phys.data)
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #extracts all observations (rows) in which the lake depth is 0

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #does same thing as line 79
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) #yields same result as lines 79 and 82
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #makes new data frame with desired results in it
...
#lines 95, 96, and 100 all yield the same result...produce a new data frame containing only the Paul Lake and Peter Lake Data
...
#lines 105, 106, 107, and 108 all yield the same result...produce a new data frame containing data for June through October only
...
NTL.phys.data.Tuesday <- filter(NTL.phys.data, 
                                lakename == "Tuesday Lake" &
                                  year4 > 1989 & year4 <2000) # or 'year4 %in% c(1990:1999))'


```
Question: Why don't we filter using row numbers?

> Answer: As you filter, row numbers will change.
...
# Which dates, lakes, and depths have the highest temperatures? Hummingbird Lake, East Long Lake, Central Long Lake; shallow depths (0-0.5m); summer (June and July) of 1995, 1998, 2002
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #selects and keeps columns from dataset; if you put a minus sign before the name of a column in the above function, it will exclude it; commands need to be either all-exclusions or all-inclusions
...
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32) #creates a new column at very end (far right) of dataset containing temperatures in F
...
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #numbers here are positions (first and last dates in dataset)
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #yields same result as line 175 (more useful because you don't have to count/look for specific positions)
```
