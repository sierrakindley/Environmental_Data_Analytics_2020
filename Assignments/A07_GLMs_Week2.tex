...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(nlme) }\CommentTok{#load nlme package}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(piecewiseSEM) }\CommentTok{#load piecewiseSEM package}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{#3}
\NormalTok{LakeChemData.raw.filtered <-}\StringTok{ }
\StringTok{  }\NormalTok{LakeChemData.raw }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(daynum }\OperatorTok{>}\StringTok{ }\DecValTok{181} \OperatorTok{&}\StringTok{ }\NormalTok{daynum }\OperatorTok{<}\StringTok{ }\DecValTok{213}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\CommentTok{#only select data from dates in July}
\StringTok{  }\KeywordTok{select}\NormalTok{(lakename, year4, daynum, depth, temperature_C) }\OperatorTok{%>%}\StringTok{ }\CommentTok{#select only the specified columns}
\StringTok{  }\KeywordTok{na.exclude}\NormalTok{() }\CommentTok{#remove incomplete cases containing NAs}

\CommentTok{#4}
\NormalTok{LakeChemData.AIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ LakeChemData.raw.filtered, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }
\StringTok{                    }\NormalTok{depth)}
\KeywordTok{step}\NormalTok{(LakeChemData.AIC) }
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ year4 + daynum + depth, data = LakeChemData.raw.filtered)
## 
## Coefficients:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#run an AIC to determine variables best suited for predicting temperature}
\NormalTok{LakesChem.multregression <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ LakeChemData.raw.filtered, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }
\StringTok{                            }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth) }\CommentTok{#run a multiple regression on recommended variables}
\KeywordTok{summary}\NormalTok{(LakesChem.multregression)}
\end{Highlighting}
\end{Shaded}
...
from my multiple regression include year4, daynum, and depth.
Approximately 74.17\% of the oberved variance is explained by this
model.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#6}
\NormalTok{LakeChem.ancova.interaction <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ LakeChemData.raw.filtered, }
\NormalTok{                                  temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\NormalTok{depth)}
\CommentTok{#run interaction effects ANCOVA to predict temperature from depth & lakename}
\KeywordTok{summary}\NormalTok{(LakeChem.ancova.interaction)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ lakename * depth, data = LakeChemData.raw.filtered)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
...
lakename. This explains approximately 78.57\% of the variance in the
...
\begin{Highlighting}[]
\CommentTok{#8}
\NormalTok{Lakes.plot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(LakeChemData.raw.filtered, }
...
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.5}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{#make points 50% transparent}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method =} \StringTok{"lm"}\NormalTok{, }\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{#add geom_smooth for each lake}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Depth (m)"}\NormalTok{, }\DataTypeTok{y =} \KeywordTok{expression}\NormalTok{(}\StringTok{"Temperature "}\NormalTok{ ( degree}\OperatorTok{~}\NormalTok{C)), }\DataTypeTok{color =} \StringTok{""}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{#axis labels}
\StringTok{  }\KeywordTok{scale_color_brewer}\NormalTok{(}\DataTypeTok{palette =} \StringTok{"Paired"}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{#define color palette}
\StringTok{  }\KeywordTok{ylim}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{35}\NormalTok{) }\CommentTok{#define y-axis limits}
\KeywordTok{print}\NormalTok{(Lakes.plot)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{LitterTest.mixed <-}\StringTok{ }\KeywordTok{lme}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
...
\CommentTok{#run mixed effects model to predict dry mass with nlcdClass and functionalGroup}
\CommentTok{#as fixed effects w/ interaction and plotID as a random effect}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rsquared}\NormalTok{(LitterTest.mixed) }\CommentTok{#check marginal & conditional R^2 values of model}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: Approximately 2\% more variance is explained by adding the
random effect to the model.
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{LitterTest.fixed <-}\StringTok{ }\KeywordTok{gls}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
\NormalTok{                      dryMass }\OperatorTok{~}\StringTok{ }\NormalTok{nlcdClass }\OperatorTok{*}\StringTok{ }\NormalTok{functionalGroup)}
\CommentTok{#run fixed effects model to predict dry mass with nlcdClass and functionalGroup}
\CommentTok{#as fixed effects w/ interaction (no random effects included)}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rsquared}\NormalTok{(LitterTest.fixed) }\CommentTok{#check R^2 value of the model}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{anova}\NormalTok{(LitterTest.mixed, LitterTest.fixed) }\CommentTok{#run ANOVA on mixed & fixed tests}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: Yes, the mixed effects model (with the random effect included)
is a better model than the fixed effects model (with no random effect),
as it is able to explain a (slightly) larger percentage of the variance
in the data and yields a (slightly) lower AIC value. The fact that the
p-value of the ANOVA (run in part 9d) is less than the significance
level indicates that we can reject the null hypothesis and say that
there is, indeed, a statistically significant difference in the fit of
the two models.
\end{quote}
