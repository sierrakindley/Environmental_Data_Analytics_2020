...
\begin{Highlighting}[]
\CommentTok{# 2 }
\NormalTok{GaringerOzone.combined <-}
\StringTok{  }\KeywordTok{rbind}\NormalTok{(GaringerOzone2010, GaringerOzone2011, GaringerOzone2012, GaringerOzone2013,}
\NormalTok{            GaringerOzone2014, GaringerOzone2015, GaringerOzone2016, GaringerOzone2017, }
\NormalTok{            GaringerOzone2018, GaringerOzone2019)}
  \CommentTok{#combine ten ozone time series datasets into single dataset}

\CommentTok{# 3}
\NormalTok{GaringerOzone.combined}\OperatorTok{$}\NormalTok{Date <-}\StringTok{ }
\StringTok{  }\KeywordTok{as.Date}\NormalTok{(GaringerOzone.combined}\OperatorTok{$}\NormalTok{Date, }\DataTypeTok{format =} \StringTok{"%m/%d/%Y"}\NormalTok{)}
  \CommentTok{#format 'Date' column as date}
\KeywordTok{class}\NormalTok{(GaringerOzone.combined}\OperatorTok{$}\NormalTok{Date) }\CommentTok{#verify class of 'Date' column }
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{na.approx}\NormalTok{(GaringerOzone}\OperatorTok{$}\NormalTok{Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration)}
  \CommentTok{#fill in missing daily ozone data using linear interpolation}

\CommentTok{# 9}
\NormalTok{GaringerOzone.monthly <-}\StringTok{ }\CommentTok{#create new data frame}
\StringTok{  }\NormalTok{GaringerOzone }\OperatorTok{%>%}\StringTok{ }\CommentTok{#pipe from 'GaringerOzone' dataset}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Year =} \KeywordTok{year}\NormalTok{(Date), }
            \DataTypeTok{Month =} \KeywordTok{month}\NormalTok{(Date)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{#add columns for month and year}
\StringTok{    }\KeywordTok{group_by}\NormalTok{(Year, Month) }\OperatorTok{%>%}\StringTok{ }\CommentTok{#group data by year and month}
\StringTok{    }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{Mean_Ozone_Concentration =} \KeywordTok{mean}\NormalTok{(Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration)) }
      \CommentTok{#generate mean ozone concentrations for each month}
...
\NormalTok{                                            GaringerOzone.monthly}\OperatorTok{$}\NormalTok{Month, }
  ...
  \CommentTok{#create new 'Date' column with each month-year combo. set as first day of month}

\CommentTok{# 10}
\NormalTok{GaringerOzone.monthly.ts <-}\StringTok{ }
\StringTok{  }\KeywordTok{ts}\NormalTok{(GaringerOzone.monthly}\OperatorTok{$}\NormalTok{Mean_Ozone_Concentration, }\DataTypeTok{frequency =} \DecValTok{12}\NormalTok{, }
  ...
  \CommentTok{#generate new time series and define monthly frequency}

\CommentTok{# 11}
\NormalTok{GaringerOzone.monthly.smk <-}\StringTok{ }
\StringTok{  }\KeywordTok{smk.test}\NormalTok{(GaringerOzone.monthly.ts)}
  \CommentTok{#run SMK time series analysis}

\NormalTok{GaringerOzone.monthly.smk}
\end{Highlighting}
\end{Shaded}
