...
summary(Neonics$Species.Common.Name) #ask R for a summary of the 'Species.Common.Name' column
#in the dataset 'Neonics'
...
class(Neonics$Conc.1..Author.) #ask R the class of the column 'Conc.1..Author.' in
#the dataset 'Neonics'
...
Litter$collectDate <- as.Date(Litter$collectDate, format = "%Y-%m-%d") #change the class of 
#collectDate to date with the format "%Y-%m-%d"
class(Litter$collectDate) #ask R what the class of collectDate is again
unique(Litter$collectDate, Litter$collectDate >= "2018-08-01" & Litter$collectDate <= "2018-08-31", incomparables = FALSE)
...
unique(Litter$plotID, incomparables = FALSE) #ask R for only the names of the plots sampled at 
#Niwot Ridge with no duplicates
...
ggplot(Litter) +
  geom_boxplot(aes(x = functionalGroup, y = dryMass)) #generate a boxplot of dryMass by functionalGroup

ggplot(Litter) +
  geom_violin(aes(x = functionalGroup, y = dryMass)) #generate a violin plot of dryMass by
#functionalGroup
